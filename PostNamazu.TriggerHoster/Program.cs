    ...
    public void Init(string[] commands) {
        ...
        foreach (var command in commands) {
            var parameters = new object[] {
                command, new ProxyPlugin.CustomCallbackDelegate((_, payload) => PostNamazuDelegate(command, payload)), null, "PostNamazu" // registrant
            };
        ...
        _triggCallbackId.Add((int)registerNamedCallbackMethod!.Invoke(_triggPlugin,
            ["NamazuLog", new ProxyPlugin.CustomCallbackDelegate((_, log) => LogDelegate(log)), null, "PostNamazu"])
        );
    }
